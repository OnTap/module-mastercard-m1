...
class Mastercard_Mpgs_Model_Method_Amex extends Mastercard_Mpgs_Model_Method_Abstract implements Mastercard_Mpgs_Model_Method_WalletInterface
{
     ...
     *
     * @return Mastercard_Mpgs_Model_Config_Amex|Mage_Core_Model_Abstract
     */
    ...
    public function capture(Varien_Object $payment, $amount)
    {
        ...
        if (empty($txnAuth)) {
            $captureInfo = $restAPI->payFromSession($order);
            $helper->updateTransferInfo($payment, $captureInfo);
            $helper->addPayTnxPayment($payment, $captureInfo);
        } else {
    ...
    public function refund(Varien_Object $payment, $amount)
    {
        ...
        $refundInfo = $restAPI->refund_order(
            $order->getIncrementId(),
            $amount,
